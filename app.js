...
async function saveClinic(e){e.preventDefault();$('formMessage').textContent='';const inpatient=isInpatient(),duration=inpatient?null:Number($('duration').value),n=Number($('newConsultations').value||0),f=Number($('followUps').value||0);if(!$('date').value||!$('clinic').value)return msg('formMessage','Please select a date and clinic/service.');if(!Number.isInteger(n)||n<0||!Number.isInteger(f)||f<0)return msg('formMessage','New consultations and follow-up patients must be whole numbers of zero or more.');if(!inpatient&&(!duration||duration<=0))return msg('formMessage','Please enter the clinic duration.');const total=inpatient?null:n+f;if(!inpatient)$('totalPatients').value=total;const item={id:$('editingId').value||uid(),date:$('date').value,day:getDay($('date').value),clinic:$('clinic').value,duration,totalPatients:total,newConsultations:n,followUps:f,updatedAt:new Date().toISOString()};if(supabaseUser){if(!navigator.onLine)return msg('formMessage','You are offline. Reconnect before saving so the cloud and this device stay consistent.');const ok=await cloudUpsertRecord('clinic_records',item);if(!ok)return msg('formMessage','Save failed. Nothing was changed locally. Please try again.')}upsert(clinicState.entries,item);persist(CLINIC_KEY,clinicState.entries);resetClinic();renderClinic();msg('formMessage','Saved successfully.')}
...
window.editClinic=editClinic;window.deleteClinic=async id=>{const key=String(id),x=clinicState.entries.find(e=>String(e.id)===key);if(!x||!confirm(`Delete the entry for ${x.clinic} on ${fmtDate(x.date)}?`))return;if(supabaseUser){if(!navigator.onLine)return alert('You are offline. Reconnect before deleting so the record cannot return during synchronization.');const ok=await cloudDeleteRecord('clinic_records',key);if(!ok)return alert('Cloud deletion failed. The record was not removed.')}createAutomaticLocalBackup('Before deleting clinic record');clinicState.entries=clinicState.entries.filter(e=>String(e.id)!==key);persist(CLINIC_KEY,clinicState.entries);renderClinic();alert('Clinic record deleted.');};
async function clearClinic(){if(!clinicState.entries.length||!confirm('Delete ALL clinic entries? This cannot be undone.'))return;if(supabaseUser){if(!navigator.onLine)return alert('You are offline. Reconnect before clearing records.');const ok=await cloudClearTable('clinic_records');if(!ok)return alert('Cloud clear failed. No clinic records were removed.')}createAutomaticLocalBackup('Before clearing clinic records');clinicState.entries=[];persist(CLINIC_KEY,[]);renderClinic();alert('All clinic records were cleared.')} 
...
async function saveEndo(e){e.preventDefault();$('endoscopyMessage').textContent='';const procedures=selectedProcedures();if(!$('endoscopyDate').value||!$('mrn').value.trim()||!$('hospital').value)return msg('endoscopyMessage','Please complete date, MRN, and hospital.');if(!procedures.length)return msg('endoscopyMessage','Select at least one procedure.');const item={id:$('endoscopyEditingId').value||uid(),date:$('endoscopyDate').value,day:getDay($('endoscopyDate').value),mrn:$('mrn').value.trim(),hospital:$('hospital').value,procedures,extras:extrasFromForm(),note:$('endoscopyNote').value.trim(),updatedAt:new Date().toISOString()};if(supabaseUser){if(!navigator.onLine)return msg('endoscopyMessage','You are offline. Reconnect before saving so the cloud and this device stay consistent.');const ok=await cloudUpsertRecord('procedure_records',item);if(!ok)return msg('endoscopyMessage','Save failed. Nothing was changed locally. Please try again.')}upsert(endoState.entries,item);persist(ENDO_KEY,endoState.entries);resetEndo();renderEndo();msg('endoscopyMessage','Saved successfully.')}
...
window.editEndo=editEndo;window.deleteEndo=async id=>{const key=String(id),e=endoState.entries.find(x=>String(x.id)===key);if(!e||!confirm(`Delete endoscopy record for MRN ${e.mrn}?`))return;if(supabaseUser){if(!navigator.onLine)return alert('You are offline. Reconnect before deleting so the record cannot return during synchronization.');const ok=await cloudDeleteRecord('procedure_records',key);if(!ok)return alert('Cloud deletion failed. The record was not removed.')}createAutomaticLocalBackup('Before deleting endoscopy record');endoState.entries=endoState.entries.filter(x=>String(x.id)!==key);persist(ENDO_KEY,endoState.entries);renderEndo();alert('Endoscopy record deleted.');};
async function clearEndo(){if(!endoState.entries.length||!confirm('Delete ALL endoscopy records? This cannot be undone.'))return;if(supabaseUser){if(!navigator.onLine)return alert('You are offline. Reconnect before clearing records.');const ok=await cloudClearTable('procedure_records');if(!ok)return alert('Cloud clear failed. No endoscopy records were removed.')}createAutomaticLocalBackup('Before clearing endoscopy records');endoState.entries=[];persist(ENDO_KEY,[]);renderEndo();alert('All endoscopy records were cleared.')} 
...
async function initSupabaseSync(){if(!window.supabase?.createClient){setSyncStatus('error','Cloud library unavailable');return}supabaseClient=window.supabase.createClient(SUPABASE_URL,SUPABASE_PUBLISHABLE_KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});const {data:{session}}=await supabaseClient.auth.getSession();if(session?.user)await handleSignedIn(session.user);else{setSyncStatus('offline','Cloud not connected');showCloudAuth()}supabaseClient.auth.onAuthStateChange((_event,session)=>{if(session?.user)handleSignedIn(session.user);else handleSignedOut()});$('cloudAuthForm').onsubmit=signInToCloud;$('cloudCreateAccountBtn').onclick=createCloudAccount;$('cloudOfflineBtn').onclick=()=>{hideCloudAuth();setSyncStatus('offline','Device-only mode')};$('syncStatusBtn').onclick=()=>supabaseUser?syncAllFromCloud(true):showCloudAuth();$('syncNowBtn').onclick=()=>syncAllFromCloud(true);$('uploadLocalDataBtn').onclick=()=>uploadAllLocalData(true);$('downloadCloudDataBtn').onclick=()=>syncAllFromCloud(true);$('signOutBtn').onclick=safeSignOut;$('headerSignOutBtn').onclick=toggleAccountMenu;$('accountSyncBtn').onclick=async()=>{closeAccountMenu();await syncAllFromCloud(true)};$('accountSignOutBtn').onclick=()=>{closeAccountMenu();safeSignOut()};document.addEventListener('click',event=>{if(!$('accountMenu')?.classList.contains('hidden')&&!event.target.closest('.account-menu-wrap'))closeAccountMenu()});document.addEventListener('keydown',event=>{if(event.key==='Escape')closeAccountMenu()});window.addEventListener('online',()=>{if(supabaseUser)setSyncStatus('online','Online — press Sync Now to refresh')});window.addEventListener('offline',()=>{if(supabaseUser)setSyncStatus('offline','Offline')});updateSyncCounts()}
...
function toggleAccountMenu(event){event?.stopPropagation();if(!supabaseUser)return showCloudAuth();$('accountMenu')?.classList.toggle('hidden')}
function closeAccountMenu(){$('accountMenu')?.classList.add('hidden')}
async function handleSignedIn(user){supabaseUser=user;hideCloudAuth();$('headerSignOutBtn')?.classList.remove('hidden');if($('accountMenuEmail'))$('accountMenuEmail').textContent=user.email||'Signed-in user';setSyncStatus('working','Connecting…');updateSyncCounts();await syncAllFromCloud(false)}
function handleSignedOut(){supabaseUser=null;closeAccountMenu();$('headerSignOutBtn')?.classList.add('hidden');if($('accountMenuEmail'))$('accountMenuEmail').textContent='—';if(supabaseChannel&&supabaseClient)supabaseClient.removeChannel(supabaseChannel);supabaseChannel=null;setSyncStatus('offline','Cloud not connected');updateSyncCounts();showCloudAuth()}
async function safeSignOut(){if(!supabaseClient||!supabaseUser)return showCloudAuth();const email=supabaseUser.email||'this account';if(!confirm(`Sign out of ${email}?\n\nYour cloud data will remain safe. This device's local cache will be cleared so another account cannot see or upload your records.`))return;try{createAutomaticLocalBackup('Before sign out');localStorage.removeItem(CLINIC_KEY);localStorage.removeItem(ENDO_KEY);localStorage.removeItem(INCOME_SETTINGS_KEY);localStorage.removeItem(INCOME_PASSWORD_KEY);localStorage.removeItem(SUPABASE_SYNC_META_KEY);clinicState.entries=[];endoState.entries=[];incomeSettings=loadIncomeSettings();incomeUnlocked=false;renderClinic();renderEndo();renderFeeSettings();renderIncomeDashboard();renderPrivateOverview();renderPrivateHospitalCards();renderPerformanceDashboard();const {error}=await supabaseClient.auth.signOut();if(error)throw error;}catch(error){console.error(error);alert(error.message||'Unable to sign out. Please try again.')}}
...
async function cloudUpsertRecord(table,record){if(!supabaseClient||!supabaseUser||!navigator.onLine)return false;setSyncStatus('working','Saving…');const {error}=await supabaseClient.from(table).upsert(cloudRowFromRecord(record),{onConflict:'id'});if(error){console.error(error);setSyncStatus('error','Save failed');return false}markSyncComplete();return true}
async function cloudDeleteRecord(table,id){if(!supabaseClient||!supabaseUser||!navigator.onLine)return false;setSyncStatus('working','Deleting…');const {data,error}=await supabaseClient.from(table).delete().eq('id',String(id)).eq('user_id',supabaseUser.id).select('id');if(error){console.error(error);setSyncStatus('error','Delete failed');return false}if(!data?.length){setSyncStatus('error','Delete not confirmed');return false}markSyncComplete();return true}
async function cloudClearTable(table){if(!supabaseClient||!supabaseUser||!navigator.onLine)return false;setSyncStatus('working','Clearing…');const {error}=await supabaseClient.from(table).delete().eq('user_id',supabaseUser.id);if(error){console.error(error);setSyncStatus('error','Clear failed');return false}const {count,error:verifyError}=await supabaseClient.from(table).select('id',{count:'exact',head:true}).eq('user_id',supabaseUser.id);if(verifyError){console.error(verifyError);setSyncStatus('error','Clear verification failed');return false}if(count!==0){setSyncStatus('error','Clear not confirmed');return false}markSyncComplete();return true}
...
async function syncAllFromCloud(showMessage=false){if(syncInProgress||!supabaseClient||!supabaseUser)return;if(!navigator.onLine){setSyncStatus('offline','Offline');if(showMessage)msg('syncMessage','You are offline.');return}syncInProgress=true;setSyncStatus('working','Refreshing…');if(showMessage)msg('syncMessage','Downloading the latest cloud data…');try{const cloud=await fetchCloudData();clinicState.entries=cloud.clinic;endoState.entries=cloud.procedures;localStorage.setItem(CLINIC_KEY,JSON.stringify(cloud.clinic));localStorage.setItem(ENDO_KEY,JSON.stringify(cloud.procedures));if(cloud.settings){if(cloud.settings.incomeSettings)localStorage.setItem(INCOME_SETTINGS_KEY,JSON.stringify(cloud.settings.incomeSettings));if(cloud.settings.passwordHash)localStorage.setItem(INCOME_PASSWORD_KEY,cloud.settings.passwordHash);if(cloud.settings.theme){localStorage.setItem(THEME_KEY,cloud.settings.theme);document.body.classList.toggle('dark',cloud.settings.theme==='dark');updateThemeButton()}incomeSettings=loadIncomeSettings();renderFeeSettings()}renderClinic();renderEndo();if(incomeUnlocked){renderIncomeDashboard();renderPrivateOverview();renderPrivateHospitalCards();renderPerformanceDashboard()}saveSyncMeta({lastSyncAt:new Date().toISOString(),cloudClinicCount:cloud.clinic.length,cloudProcedureCount:cloud.procedures.length});setSyncStatus('online','Cloud refreshed');updateSyncCounts(cloud.clinic.length,cloud.procedures.length);if(showMessage)msg('syncMessage','Cloud data downloaded successfully.')}catch(err){console.error(err);setSyncStatus('error','Refresh failed');if(showMessage)msg('syncMessage',err.message||'Cloud refresh failed.')}finally{syncInProgress=false}}
...
function subscribeToCloud(){/* Real-time background synchronization intentionally disabled for stability. */}
