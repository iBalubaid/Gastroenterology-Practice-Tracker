...
function addPendingToCalendar(id){
  ...
  const title=`${(item.procedures||[]).join(' + ')||'Endoscopy'} · MRN ${item.mrn}`;
  const description=[`MRN: ${item.mrn}`,`Hospital: ${item.hospital}`,`Procedures: ${(item.procedures||[]).join(', ')}`,item.note?`Notes: ${item.note}`:''].filter(Boolean).join('\n');
  const stamp=new Date().toISOString().replace(/[-:]/g,'').replace(/\.\d{3}Z$/,'Z');
  const ics=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//Gastroenterology Practice Tracker//EN','CALSCALE:GREGORIAN','METHOD:PUBLISH','BEGIN:VEVENT',`UID:pending-${icsEscape(item.id)}@practice-tracker`,`DTSTAMP:${stamp}`,`DTSTART:${start}`,`DTEND:${end}`,`SUMMARY:${icsEscape(title)}`,`LOCATION:${icsEscape(item.hospital)}`,`DESCRIPTION:${icsEscape(description)}`,'END:VEVENT','END:VCALENDAR'].join('\r\n');
  const blob=new Blob([ics],{type:'text/calendar;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`${item.date}-${String(item.mrn).replace(/[^a-z0-9]/gi,'')}-endoscopy.ics`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1500);
...
function savePending(e){
  ...
  try{const existing=pendingState.entries.find(x=>String(x.id)===editingId);const item={id:$('pendingEditingId').value||uid(),date:$('pendingDate').value,time:$('pendingTime').value||'',durationMinutes:Number($('pendingDuration').value)||calculatedPendingDuration(procedures),mrn:$('pendingMrn').value.trim(),hospital:pendingState.hospital,procedures,note:$('pendingNote').value.trim(),createdAt:existing?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};if(existing&&existing.calendarFingerprint===pendingCalendarFingerprint(item)){item.calendarAddedAt=existing.calendarAddedAt;item.calendarFingerprint=existing.calendarFingerprint}upsert(pendingState.entries,item);persist(PENDING_KEY,pendingState.entries);createAutomaticLocalBackup('Pending patient saved');togglePendingComposer(false);renderPending();finishSave(button,label,'Pending patient saved')}
