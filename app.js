...
function savePending(e){
  ...
  try{const existing=pendingState.entries.find(x=>String(x.id)===editingId);const item={id:$('pendingEditingId').value||uid(),date:$('pendingDate').value,time:$('pendingTime').value||'',durationMinutes:Number($('pendingDuration').value)||calculatedPendingDuration(procedures),mrn:$('pendingMrn').value.trim(),hospital:pendingState.hospital,procedures,note:$('pendingNote').value.trim(),createdAt:existing?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString(),calendarAddedAt:existing?.calendarAddedAt||'',calendarFingerprint:existing?.calendarFingerprint||''};upsert(pendingState.entries,item);persist(PENDING_KEY,pendingState.entries);createAutomaticLocalBackup('Pending patient saved');togglePendingComposer(false);renderPending();finishSave(button,label,'Pending patient saved')}
...
function resetPending(resetDate=true){pendingForm.reset();$('pendingEditingId').value='';$('pendingHospital').value=pendingState.hospital;if(resetDate)$('pendingDate').value=today();$('pendingDuration').value='30';$('pendingSubmitLabel').textContent='Add to pending';$('pendingMessage').textContent=''}
function editPending(id){const item=pendingState.entries.find(x=>String(x.id)===String(id));if(!item)return;togglePendingComposer(true);if(item.hospital!==pendingState.hospital)setPendingHospital(item.hospital);$('pendingEditingId').value=item.id;$('pendingDate').value=item.date;$('pendingTime').value=item.time||'';$('pendingDuration').value=String(item.durationMinutes||calculatedPendingDuration(item.procedures||[]));$('pendingMrn').value=item.mrn;$('pendingNote').value=item.note||'';document.querySelectorAll('#pendingProcedureChoices input').forEach(x=>x.checked=(item.procedures||[]).includes(x.value));$('pendingSubmitLabel').textContent='Update pending';scrollTo(0,0)}
