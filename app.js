...
function resetPending(resetDate=true){pendingForm.reset();$('pendingEditingId').value='';$('pendingHospital').value=pendingState.hospital;$('pendingToBeScheduled').checked=false;if(resetDate)$('pendingDate').value=today();$('pendingDuration').value='30';syncPendingScheduleFields();$('pendingSubmitLabel').textContent='Add to pending';$('pendingMessage').textContent=''}
...
function saveInlineBooking(id){
  ...
  if(!date||!time||duration<5){if(message)message.textContent='Please select a date, start time, and duration.';return}
  item.date=date;item.time=time;item.durationMinutes=duration;item.toBeScheduled=false;item.calendarAddedAt='';item.calendarFingerprint='';item.updatedAt=new Date().toISOString();
