...
function togglePendingComposer(open){const composer=$('pendingComposer');if(!composer)return;composer.classList.toggle('open',Boolean(open));document.body.classList.toggle('pending-composer-open',Boolean(open));if(open){setTimeout(()=>($('pendingMrn')||{}).focus?.(),80)}else resetPending()}
