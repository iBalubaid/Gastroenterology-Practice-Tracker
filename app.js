...
const pendingState={entries:load(PENDING_KEY),hospital:"HMG Fayhaa",search:"",view:"queue",expandedId:"",bookingId:""};
...
$('date').onchange=()=>updateDay('date','day');document.querySelectorAll('[data-clinic-hospital]').forEach(btn=>btn.onclick=()=>selectClinicHospital(btn.dataset.clinicHospital));$('durationHours').onchange=syncClinicDurationPicker;$('durationMinutes').onchange=syncClinicDurationPicker;document.querySelectorAll('[data-consultation-type]').forEach(btn=>btn.onclick=()=>selectClinicType(btn.dataset.consultationType));$('newConsultations').oninput=updatePatientTotal;$('followUps').oninput=updatePatientTotal;$('inpatientConsultations').oninput=updatePatientTotal;clinicForm.onsubmit=saveClinic;$('resetFormBtn').onclick=resetClinic;$('exportBtn').onclick=exportClinic;$('clearAllBtn').onclick=clearClinic;
...
function syncClinicDurationPicker(){const hours=$('durationHours').value;if(hours===''){$('duration').value='';$('durationMinutes').value='0';$('durationMinutes').disabled=true;return}$('durationMinutes').disabled=false;const minutes=Number($('durationMinutes').value||0);$('duration').value=String(Number(hours)+(minutes/60))}
function selectClinicDuration(value){if(value===''||value==null){$('durationHours').value='';$('durationMinutes').value='0';syncClinicDurationPicker();return}const totalMinutes=Math.round(Number(value)*60);$('durationHours').value=String(Math.floor(totalMinutes/60));$('durationMinutes').value=String(totalMinutes%60);syncClinicDurationPicker()}
...
function syncPendingScheduleFields(){
  ...
  if(date){date.required=!unscheduled;date.disabled=unscheduled;if(unscheduled)date.value='';else if(!date.value)date.value=today()}
  if(time){time.disabled=unscheduled;if(unscheduled)time.value=''}
  if(duration)duration.disabled=unscheduled;
}
function togglePendingComposer(open){const composer=$('pendingComposer');if(!composer)return;composer.classList.remove('case-details-only');const shouldOpen=Boolean(open);composer.classList.toggle('open',shouldOpen);composer.hidden=false;composer.setAttribute('aria-hidden',String(!shouldOpen));document.body.classList.toggle('pending-composer-open',shouldOpen);if(shouldOpen){composer.scrollTop=0;setTimeout(()=>{const mrn=$('pendingMrn');if(mrn){mrn.focus({preventScroll:true});mrn.scrollIntoView({block:'center'})}},80)}else resetPending()}
...
function calculatedPendingDuration(procedures){
  const selected=new Set(procedures||[]);let total=0;
  if(selected.has('EGD')&&selected.has('Colonoscopy')){total+=60;selected.delete('EGD');selected.delete('Colonoscopy')}
  const minutes={'EGD':30,'Colonoscopy':45,'Flex Sig':30,'EUS':120,'ERCP':120,'Fibroscan':20,'PEG Tube Insertion':45,'PEG Tube Replacement':30,'Foreign Body Removal':45,'pH Monitoring':20};
  selected.forEach(name=>{total+=minutes[name]||30});return total||30
}
function updatePendingDuration(){$('pendingDuration').value=String(calculatedPendingDuration(pendingProcedures()))}
function pendingCalendarFingerprint(item){return JSON.stringify({date:item.date||'',time:item.time||'',hospital:item.hospital||'',procedures:[...(item.procedures||[])].sort(),duration:Number(item.durationMinutes)||0})}
function pendingCalendarStatus(item){if(!item.calendarAddedAt)return'new';return item.calendarFingerprint===pendingCalendarFingerprint(item)?'added':'update'}
function pendingHospitalCode(hospital){return hospital==='HMG Mohammadiya'?'M':'F'}
function pendingCalendarProcedureTitle(procedures){
  const selected=[...(procedures||[])],parts=[];
  const hasEgd=selected.includes('EGD'),hasColon=selected.includes('Colonoscopy');
  if(hasEgd&&hasColon)parts.push('Double');
  else{if(hasEgd)parts.push('EGD');if(hasColon)parts.push('Colon')}
  const labels={'Flex Sig':'Flex Sig','EUS':'EUS','ERCP':'ERCP','Fibroscan':'FibroScan','PEG Tube Insertion':'PEG','PEG Tube Replacement':'PEG Replace','Foreign Body Removal':'Foreign Body','pH Monitoring':'pH'};
  selected.filter(p=>p!=='EGD'&&p!=='Colonoscopy').forEach(p=>parts.push(labels[p]||p));
  return parts.join(' + ')||'Endoscopy'
}
...
function openPendingBook(id){
  ...
  const modal=$('pendingBookModal');if(!modal)return;modal.hidden=false;modal.removeAttribute('hidden');modal.classList.add('open');modal.style.display='flex';modal.setAttribute('aria-hidden','false');document.body.classList.add('pending-book-open');
  setTimeout(()=>$('pendingBookDate').focus(),50);
}
function closePendingBook(){
  const modal=$('pendingBookModal');if(!modal)return;modal.classList.remove('open');modal.hidden=true;modal.setAttribute('hidden','');modal.style.display='none';modal.setAttribute('aria-hidden','true');document.body.classList.remove('pending-book-open');
}
function savePendingBook(event){
  ...
  if(!date||duration<5)return validationMessage('pendingBookMessage','Please select a date and duration. Start time is optional.');
...
function editPendingDetails(id){
  // Keep this alias for backward compatibility, but always open the full editor.
  editPending(id);
}
...
function savePending(e){
  ...
  const existing=pendingState.entries.find(x=>String(x.id)===editingId);
  const requestedBooking=Boolean($('pendingToBeScheduled').checked);
  const hasPlannedDate=Boolean($('pendingDate').value);
  const toBeScheduled=requestedBooking||Boolean(existing?.toBeScheduled&&!hasPlannedDate);
  if(!$('pendingMrn').value.trim())return validationMessage('pendingMessage','Please enter an MRN.');
  if(!toBeScheduled&&!hasPlannedDate)return validationMessage('pendingMessage','Please enter a planned date or mark the case To be booked.');
  ...
  try{const item={id:$('pendingEditingId').value||uid(),date:toBeScheduled?'':$('pendingDate').value,time:toBeScheduled?'':($('pendingTime').value||''),durationMinutes:toBeScheduled?0:(Number($('pendingDuration').value)||calculatedPendingDuration(procedures)),toBeScheduled,mrn:$('pendingMrn').value.trim(),hospital:pendingState.hospital,procedures,note:$('pendingNote').value.trim(),createdAt:existing?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString(),calendarAddedAt:toBeScheduled?'':(existing?.calendarAddedAt||''),calendarFingerprint:toBeScheduled?'':(existing?.calendarFingerprint||'')};upsert(pendingState.entries,item);persist(PENDING_KEY,pendingState.entries);createAutomaticLocalBackup('Pending patient saved');togglePendingComposer(false);renderPending();finishSave(button,label,toBeScheduled?'Case kept in Booking':'Pending patient scheduled')}
...
function resetPending(resetDate=true){$('pendingComposer')?.classList.remove('case-details-only');pendingForm.reset();$('pendingEditingId').value='';$('pendingHospital').value=pendingState.hospital;$('pendingToBeScheduled').checked=false;if(resetDate)$('pendingDate').value=today();$('pendingDuration').value='30';syncPendingScheduleFields();$('pendingSubmitLabel').textContent='Add to pending';$('pendingMessage').textContent=''}
function editPending(id){const item=pendingState.entries.find(x=>String(x.id)===String(id));if(!item)return;togglePendingComposer(true);if(item.hospital!==pendingState.hospital)setPendingHospital(item.hospital);$('pendingEditingId').value=item.id;
  // Booking cases open with scheduling fields enabled. Add a date to move the case out of Booking; leave it blank to keep it there.
  $('pendingToBeScheduled').checked=false;
  $('pendingDate').value=item.date||'';$('pendingTime').value=item.time||'';$('pendingDuration').value=String(item.durationMinutes||calculatedPendingDuration(item.procedures||[]));syncPendingScheduleFields();
  if(item.toBeScheduled)$('pendingDate').value='';
  $('pendingMrn').value=item.mrn;$('pendingNote').value=item.note||'';document.querySelectorAll('#pendingProcedureChoices input').forEach(x=>x.checked=(item.procedures||[]).includes(x.value));$('pendingSubmitLabel').textContent=item.toBeScheduled?'Save booking':'Update pending';scrollTo(0,0)}
...
function movePendingToSchedule(id){const item=pendingState.entries.find(x=>String(x.id)===String(id));if(!item)return;const label=item.date?fmtDate(item.date):'the current date';if(!confirm(`Move MRN ${item.mrn} from ${label} to Booking?\n\nThe date and time will be cleared. Other case details will be preserved.`))return;createAutomaticLocalBackup('Before moving pending case to Schedule');item.toBeScheduled=true;item.date='';item.time='';item.durationMinutes=0;item.calendarAddedAt='';item.calendarFingerprint='';item.updatedAt=new Date().toISOString();persist(PENDING_KEY,pendingState.entries);renderPending();showToast('Case moved to Booking')}
...
function pendingTimeOptions(selected=''){
  const values=[];
  for(let mins=8*60;mins<24*60;mins+=15){
    const h=String(Math.floor(mins/60)).padStart(2,'0'),m=String(mins%60).padStart(2,'0');values.push(`${h}:${m}`)
  }
  values.push('00:00');
  return `<option value="">Select time</option>`+values.map(v=>`<option value="${v}" ${v===selected?'selected':''}>${v}</option>`).join('')
}
function showInlineBooking(id){
  pendingState.bookingId=String(id);pendingState.expandedId=String(id);renderPending();
  setTimeout(()=>{const el=document.getElementById(`inlineBookDate-${id}`);if(el){el.scrollIntoView({behavior:'smooth',block:'center'});el.focus()}},40)
}
function cancelInlineBooking(){pendingState.bookingId='';renderPending()}
function saveInlineBooking(id){
  const item=pendingState.entries.find(x=>String(x.id)===String(id));if(!item)return;
  const date=document.getElementById(`inlineBookDate-${id}`)?.value||'';
  const time=document.getElementById(`inlineBookTime-${id}`)?.value||'';
  const duration=Number(document.getElementById(`inlineBookDuration-${id}`)?.value||0);
  const message=document.getElementById(`inlineBookMessage-${id}`);
  if(!date||duration<5){if(message)message.textContent='Please select a date and duration. Start time is optional.';return}
  item.date=date;item.time=time;item.durationMinutes=duration;item.toBeScheduled=false;item.calendarAddedAt='';item.calendarFingerprint='';item.updatedAt=new Date().toISOString();
  persist(PENDING_KEY,pendingState.entries);createAutomaticLocalBackup('Pending patient booked');pendingState.bookingId='';renderPending();showToast('Patient booked')
}
function renderPending(){
  ...
  const viewLabels={queue:'Full queue',today:`Today · ${fmtDate(todayDate)}`,week:`This week · ${fmtDate(week.start)}–${fmtDate(week.end)}`,toschedule:'Cases awaiting booking'};
  if($('pendingViewSummary'))$('pendingViewSummary').textContent=viewLabels[pendingState.view];
  const emptyLabels={queue:'Add a planned patient above.',today:'No patients are scheduled for today.',week:'No patients are scheduled for this week.',toschedule:'No cases are awaiting booking.'};
