...
applyTheme();$('themeToggle').onclick=()=>{document.body.classList.toggle('dark');localStorage.setItem(THEME_KEY,document.body.classList.contains('dark')?'dark':'light');updateThemeButton()};
...
window.editClinic=editClinic;window.deleteClinic=id=>{const key=String(id),x=clinicState.entries.find(e=>String(e.id)===key);if(x&&confirm(`Delete the entry for ${x.clinic} on ${fmtDate(x.date)}?`)){createAutomaticLocalBackup('Before deleting clinic entry');clinicState.entries=clinicState.entries.filter(e=>String(e.id)!==key);persist(CLINIC_KEY,clinicState.entries);renderClinic();alert('Clinic entry deleted.')}};
...
function savePending(e){e.preventDefault();$('pendingMessage').textContent='';const procedures=pendingProcedures();if(!$('pendingDate').value||!$('pendingMrn').value.trim())return msg('pendingMessage','Please enter a date and MRN.');if(!procedures.length)return msg('pendingMessage','Select at least one planned procedure.');const item={id:$('pendingEditingId').value||uid(),date:$('pendingDate').value,mrn:$('pendingMrn').value.trim(),hospital:pendingState.hospital,procedures,note:$('pendingNote').value.trim(),createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};upsert(pendingState.entries,item);persist(PENDING_KEY,pendingState.entries);createAutomaticLocalBackup('Pending patient saved');resetPending();renderPending()}
...
function startPending(id){const item=pendingState.entries.find(x=>String(x.id)===String(id));if(!item)return;resetEndo();activePendingId=String(item.id);$('endoscopyDate').value=item.date||today();updateDay('endoscopyDate','endoscopyDay');$('mrn').value=item.mrn;$('hospital').value=item.hospital;$('endoscopyNote').value=item.note||'';document.querySelectorAll('#procedureChoices input, #additionalProcedureChoices input[type="checkbox"][value]').forEach(x=>x.checked=(item.procedures||[]).includes(x.value));$('endoscopySubmitLabel').textContent='Complete procedure';activateModule('endoscopyModule');scrollTo(0,0)}
function chooseFromPending(){activateModule('pendingModule')}
function renderPending(){const f=pendingState.entries.filter(x=>x.hospital==='HMG Fayhaa').length,m=pendingState.entries.filter(x=>x.hospital==='HMG Mohammadiya').length;$('pendingFayhaaCount').textContent=f;$('pendingMohammadiyaCount').textContent=m;const q=pendingState.search,list=pendingState.entries.filter(x=>x.hospital===pendingState.hospital&&(!q||`${x.mrn} ${(x.procedures||[]).join(' ')} ${x.note||''}`.toLowerCase().includes(q))).sort((a,b)=>(a.date||'').localeCompare(b.date||'')||(a.createdAt||'').localeCompare(b.createdAt||''));$('pendingList').innerHTML='';$('pendingEmptyState').classList.toggle('hidden',list.length>0);list.forEach((x,i)=>$('pendingList').insertAdjacentHTML('beforeend',`<article class="pending-item"><div class="pending-order">${i+1}</div><div class="pending-main"><div class="pending-meta"><strong>MRN ${esc(x.mrn)}</strong><span>${fmtDate(x.date)}</span></div><div class="pending-tags">${(x.procedures||[]).map(p=>`<span class="tag">${esc(p)}</span>`).join('')}</div>${x.note?`<p>${esc(x.note)}</p>`:''}</div><div class="pending-actions"><button class="primary-btn compact-btn" type="button" onclick="startPending('${x.id}')">Start</button><button class="icon-btn" type="button" onclick="editPending('${x.id}')">Edit</button><button class="icon-btn delete" type="button" onclick="deletePending('${x.id}')">Delete</button></div></article>`))}
window.editPending=editPending;window.deletePending=deletePending;window.startPending=startPending;
