...
function openPendingBook(id){
  ...
  const modal=$('pendingBookModal');modal.hidden=false;modal.removeAttribute('hidden');modal.style.display='flex';modal.setAttribute('aria-hidden','false');document.body.classList.add('pending-book-open');
  setTimeout(()=>$('pendingBookDate').focus(),50);
}
function closePendingBook(){
  const modal=$('pendingBookModal');modal.hidden=true;modal.setAttribute('hidden','');modal.style.display='none';modal.setAttribute('aria-hidden','true');document.body.classList.remove('pending-book-open');
}
