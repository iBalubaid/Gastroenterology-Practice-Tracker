...
function savePending(e){
  ...
  const toBeScheduled=Boolean($('pendingToBeScheduled').checked);if(!$('pendingMrn').value.trim())return validationMessage('pendingMessage','Please enter an MRN.');if(!toBeScheduled&&!$('pendingDate').value)return validationMessage('pendingMessage','Please enter a planned date or mark the case To be scheduled.');
...
function editPending(id){const item=pendingState.entries.find(x=>String(x.id)===String(id));if(!item)return;togglePendingComposer(true);if(item.hospital!==pendingState.hospital)setPendingHospital(item.hospital);$('pendingEditingId').value=item.id;$('pendingToBeScheduled').checked=Boolean(item.toBeScheduled);$('pendingDate').value=item.date||'';$('pendingTime').value=item.time||'';$('pendingDuration').value=String(item.durationMinutes||calculatedPendingDuration(item.procedures||[]));syncPendingScheduleFields();$('pendingMrn').value=item.mrn;$('pendingNote').value=item.note||'';document.querySelectorAll('#pendingProcedureChoices input').forEach(x=>x.checked=(item.procedures||[]).includes(x.value));$('pendingSubmitLabel').textContent='Update pending';scrollTo(0,0)}
