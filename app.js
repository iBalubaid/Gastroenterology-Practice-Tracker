...
function renderClinicRows(){const list=[...clinicState.entries].filter(e=>(!clinicState.filter||e.clinic===clinicState.filter)&&(!clinicState.search||`${e.date} ${e.clinic}`.toLowerCase().includes(clinicState.search))).sort(sortDesc);$('recordsBody').innerHTML='';$('emptyState').classList.toggle('hidden',list.length>0);list.forEach(e=>{$('recordsBody').insertAdjacentHTML('beforeend',`<tr><td data-label="Date">${fmtDate(e.date)}</td><td data-label="Day">${e.day||getDay(e.date)}</td><td data-label="Clinic / Service"><span class="service-badge ${isInpatient(e.clinic)?'inpatient-badge':''}">${esc(e.clinic)}</span></td><td data-label="Duration">${e.duration==null?'—':e.duration+' hr'}</td><td data-label="Total patients">${e.totalPatients??'—'}</td><td data-label="New">${e.newConsultations}</td><td data-label="Follow-up">${e.followUps}</td><td data-label="Actions" class="actions-cell"><button type="button" class="icon-btn" data-action="edit-clinic" data-id="${esc(e.id)}">Edit</button><button type="button" class="icon-btn delete" data-action="delete-clinic" data-id="${esc(e.id)}">Delete</button></td></tr>`)})}
async function deleteClinic(id){const x=clinicState.entries.find(e=>String(e.id)===String(id));if(!x||!confirm(`Delete the entry for ${x.clinic} on ${fmtDate(x.date)}?`))return;clinicState.entries=clinicState.entries.filter(e=>String(e.id)!==String(id));persist(CLINIC_KEY,clinicState.entries);renderClinic();await cloudDeleteRecord('clinic_records',id)}
window.editClinic=editClinic;window.deleteClinic=deleteClinic;
$('recordsBody').addEventListener('click',event=>{const button=event.target.closest('button[data-action]');if(!button)return;const {action,id}=button.dataset;if(action==='edit-clinic')editClinic(id);if(action==='delete-clinic')deleteClinic(id)});
...
async function handleSignedIn(user){supabaseUser=user;hideCloudAuth();$('headerSignOutBtn')?.classList.remove('hidden');if($('accountMenuEmail'))$('accountMenuEmail').textContent=user.email||'Signed-in user';setSyncStatus('working','Connecting…');updateSyncCounts();await syncAllFromCloud(false);subscribeToCloud()}
